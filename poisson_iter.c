...
void SIMD_Load_registers(double* curr, double* source, int N, int k, int j, int i, __m256d* curr_vec1, __m256d* curr_vec2, __m256d* curr_vec3, __m256d* curr_vec4, __m256d* curr_vec5, __m256d* curr_vec6, __m256d* source_vec) {
    *curr_vec1 = _mm256_loadu_pd(&curr[intex_SIMD(N, k, j, i + 1)]);
    *curr_vec2 = _mm256_loadu_pd(&curr[intex_SIMD(N, k, j, i - 1)]);
    *curr_vec3 = _mm256_loadu_pd(&curr[intex_SIMD(N, k, j + 1, i)]);
    *curr_vec4 = _mm256_loadu_pd(&curr[intex_SIMD(N, k, j - 1, i)]);
    *curr_vec5 = _mm256_loadu_pd(&curr[intex_SIMD(N, k + 1, j, i)]);
    *curr_vec6 = _mm256_loadu_pd(&curr[intex_SIMD(N, k - 1, j, i)]);
    *source_vec = _mm256_loadu_pd(&source[intex_SIMD(N, k, j, i)]);
}
...
void SIMD_Add_registers(__m256d* sum_vec, __m256d curr_vec1, __m256d curr_vec2, __m256d curr_vec3, __m256d curr_vec4, __m256d curr_vec5, __m256d curr_vec6, __m256d delta_vec, __m256d source_vec, __m256d six_vec) {
    *sum_vec = _mm256_add_pd(curr_vec1, curr_vec2);
    ...
    for (int k = slice_3D.k_start; k < slice_3D.k_end; k++) {
        for (int j = j_start; j < j_end; j++) {
            ...
            for (i = i_start; i <= i_end - 4; i += 4) {
                ...
                SIMD_Load_registers(curr, source, N, k, j, i, &curr_vec1, &curr_vec2, &curr_vec3, &curr_vec4, &curr_vec5, &curr_vec6, &source_vec);
                ...
                SIMD_Add_registers(&sum_vec, curr_vec1, curr_vec2, curr_vec3, curr_vec4, curr_vec5, curr_vec6, delta_vec, source_vec, six_vec);
                ...
                SIMD_Store_registers(next, N, k, j, i, sum_vec);
                // _mm256_storeu_pd(&next[intex_SIMD(N, k, j, i)], sum_vec);
            }
