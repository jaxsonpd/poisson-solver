...
The AVX2 instruction set allows for 256-bit wide registers to be used, which can hold four 64-bit double precision floating point numbers. This can be applied to the poisson algorithm by performing the same operation on 4 nodes simultaneously. This is done across four nodes in the i dimension.
...
It is clear that the principle of SIMD does same some CPU time for the execution of the poisson algoirthm, but the time taken to set up the calculation outweighs the time saved compared to the normal approach. Thus, the SIMD implementation was not included in the final poisson solver code.
