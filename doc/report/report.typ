...
Execution on a CPU is optimised using pipelining which allows multiple instructions to be excuted in parallel on a single core. It does this by loading the next set of instructions while the previous instructions are being executed. When branch instructions occur a control hazard develops as the CPU cannot predict which set of instructions to load. This can cause costly pipeline flushes if the CPU loads the wrong. To attempt to mitigate the number of times this pipeline flush occurs the CPU predicts which way execution will go before the conditional branch is executed. This is branch prediction and is implemented in various ways across CPU architectures.
...
V_{i:i+4,j,k,n+1}
$
A calculation is manually performed at the end of the calculation if the inner slice is not divisble by 4 to finish off the remaining nodes in the i dimention. 
